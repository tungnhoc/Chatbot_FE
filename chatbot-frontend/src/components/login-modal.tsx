...
export default function LoginModal({ onClose, onSuccess, onSwitchToSignup }: LoginModalProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Đăng nhập với Google thất bại")
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-slate-800 rounded-2xl p-10 max-w-lg w-full mx-4 border border-slate-700 shadow-xl">
        ...
        <p className="text-center text-slate-400 text-base mt-6">
          Chưa có tài khoản?{" "}
          <button onClick={onSwitchToSignup} className="text-emerald-400 hover:text-emerald-300 transition-colors font-medium">
            Đăng ký ngay
          </button>
        </p>
      </div>
